...
export async function POST(request: NextRequest) {
  try {
    ...
    if (filePath) {
      ...
      console.log(`[extract-revisions] Attempting PDF extraction — bucket="documents" filePath="${filePath}" method="storage.download()"`);
      ...
      if (pdfResult.extraction_success && pdfResult.text.length > 0) {
        ...
        console.log(
          `[extract-revisions] PDF extraction succeeded — ${pdfResult.extracted_text_length} chars, ${pdfResult.page_count} pages`
        );
        ...
        console.warn(
          `[extract-revisions] PDF extraction failed (${pdfResult.extraction_error}), falling back to provided documentText`
        );
      }
    } else if (documentText && documentText.length > 0) {
      resolvedText = documentText;
    ...
    const result = await extractAndStoreRevisions(
      resolvedText,
    ...
    const message = err instanceof Error ? err.message : 'Revision extraction failed';
